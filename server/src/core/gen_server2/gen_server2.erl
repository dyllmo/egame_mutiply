...
wake_hib(GS2State = #gs2_state { timeout_state = TS }) ->
	TimeoutState1 = case TS of
						...
						{SleptAt, TimeoutState} ->
							adjust_timeout_state(SleptAt, erlang:monotonic_time(), TimeoutState)
					end,
...
hibernate(GS2State = #gs2_state { timeout_state = TimeoutState }) ->
	TS = case TimeoutState of
			 undefined             -> undefined;
			 {backoff, _, _, _, _} -> {erlang:monotonic_time(), TimeoutState}
		 end,
...
%% 调整timeout时间的大小
adjust_timeout_state(SleptAt, AwokeAt, {backoff, CurrentTO, MinimumTO, DesiredHibPeriod, RandomState}) ->
	%% 计算出该gen_server2进程的睡眠时间微秒为单位
	NapLengthMicros = erlang:convert_time_unit(AwokeAt - SleptAt,native,micro_seconds),
	%% 得到当前睡眠的微秒数
