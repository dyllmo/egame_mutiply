...
-record(
	state, 
	{
		conn_pools = gb_trees:empty(),	%% gb_tree mapping connection pool id to a connection pool tuple
		pids_pools = gb_trees:empty(), 	%% gb_tree mapping connection Pid  to pool id  
		log_fun,						%% function for logging
		prepares = gb_trees:empty() 	%% maps names to {Statement::binary(), Version::integer()} values
	}
).
...
start_link () ->
	Id = lib_misc:get_env_atom(mysql_database),
    LogFun = fun(Level, Msg, Args) -> ?LOG(Level,Msg ++ "~n",Args) end,
	Host = lib_misc:get_env_str(mysql_host,"127.0.0.1"),
	Port = lib_misc:get_env_int(mysql_port,3306),
	UserName = lib_misc:get_env_str(mysql_username,"root"),
	Password = lib_misc:get_env_str(mysql_password,"mjmjmj"),
    start_link(Id,Host,Port,UserName,Password,atom_to_list(Id),LogFun,utf8).
    ...
    case mysql_conn:start(Host,Port,User,Password,Database, LogFun1,Encoding,PoolId) of
		...
		{error,Reason} ->
			mysql:log(
				LogFun1,
				error,
				"starting first mysql connection handler failed:~n"
				"  reason => ~p",
				[Reason]
			),
			{stop,{error,Reason}}
...
handle_call({add_conn, Conn}, _From, State) ->
	...
	mysql:log(
		State #state.log_fun,
		info,
		"added connection to mysql list:~n"
		"  pool_id => ~p~n"
		"  pid     => ~p",
		[PoolId,ConnPid]
	),
...
handle_cast ({unprepare, Name}, State) ->
	...
	mysql:log(LogFun,debug,"received unprepare/1:~p",[Name]),
	Prepares = State #state.prepares,
    State1 = case gb_trees:lookup(Name,Prepares) of
	    none ->
			mysql:log(LogFun,debug,"trying to unprepare a non-existing statement: ~p",[Name]),
			State;
	    {value,_Stmt} ->
			State #state{prepares = gb_trees:delete(Name,Prepares)}
	end,
    {noreply,State1}.
    ...
    case remove_conn(Pid,State) of
		...
		error ->	
			mysql:log(
				LogFun,
				error,
				"received 'DOWN' signal from pid ~p not in my list",
				[Pid]
			),
			{noreply,State}
    end.
		...
		{value,PoolId} ->
			...
			case gb_trees:lookup(PoolId,ConnPools) of
				...
				{value,{Unused,Used}} ->
					...
					{ok,Conn,State#state{conn_pools = NewConnPools,pids_pools = gb_trees:delete(Pid,PidsPools)}}
			end
    end.
...
get_next_conn (PoolId, State) ->
	...
	%% connections. When we run out of unused connections, we recycle
		...
		{value,{[],Used}} ->
			...
			{ok,Conn,State #state{conn_pools = gb_trees:enter(PoolId,{Conns,[Conn]},ConnPools)}};
		{value,{[Conn|Unused],Used}} ->
			{ok,Conn,State #state{conn_pools = gb_trees:enter(PoolId,{Unused,[Conn|Used]},ConnPools)}}
    end.

start_reconnect (Conn, LogFun) ->
    Pid = spawn(fun () -> reconnect_loop(Conn #conn{pid = undefined},LogFun,0) end),
    {PoolId,Host,Port} = {Conn #conn.pool_id,Conn #conn.host,Conn #conn.port},
    mysql:log(
		LogFun, 
		info,
		"mysql try reconnect:~n"
		"  pid     => ~p~n"
		"  pool_id => ~p~n"
		"  host    => ~p~n"
		"  port    => ~p~n"
    ...
    case connect(
		...
		{ok,ConnPid} ->
			mysql:log(
				LogFun, 
				info,
				"managed to reconnect:~n"
				"  pool_id => ~p~n"
				"  host    => ~p~n"
				"  port    => ~p~n"
				"  conn_id => ~p",
				[PoolId,Host,Port,ConnPid]
			),
			ok;
		{error,Reason} ->
			...
			NewN = case N of
				10 ->
					mysql:log(
						LogFun, 
						info,
						"still unable to reconnect:~n"
						"  pool_id => ~p~n"
						"  host    => ~p~n"
						"  port    => ~p~n"
						"  reason  => ~p",
						[PoolId,Host,Port,Reason]
					),
					0;
				_ ->
				   N + 1
...
encode ({{Year, Month, Day}, {Hour, Minute, Second}}, false) ->
    ...
    lists:flatten(Res);
encode ({TimeType, Val}, AsBinary) when 
	TimeType == 'date';
	TimeType == 'time' 
->
    encode(Val,AsBinary);
