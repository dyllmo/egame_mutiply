...
start (Host, Port, User, Password, Database, LogFun, Encoding, PoolId) ->
	ConnPid = self(),
    Pid = spawn(fun () -> init(Host,Port,User,Password,Database,LogFun,Encoding,PoolId,ConnPid) end),
    post_start(Pid,LogFun).

start_link (Host, Port, User, Password, Database, LogFun, Encoding, PoolId) ->
	ConnPid = self(),
    Pid = spawn_link(fun () -> init(Host,Port,User,Password,Database,LogFun,Encoding,PoolId,ConnPid) end),
    post_start(Pid,LogFun).
...
%%--------------------------------------------------------------------
do_recv (LogFun, RecvPid, SeqNum)  when 
	is_function(LogFun);
	LogFun == undefined,
	SeqNum == undefined 
->
...
loop (State) ->
    ...
    receive
		...
		{execute,Name,Version,Params,From} ->
			State1 = case do_execute(State,Name,Params,Version) of
				{error, _} = Err ->
					send_reply(From, Err),
					State;
				{ok, Result, NewState} ->
		...
		{mysql_recv,RecvPid,data,_Packet,_Num} ->
			% mysql:log(
				% LogFun, 
				% debug,
				% "mysql received not expecting data:~n"
				% "  data => ~p~n"
				% "  size => ~p",
				% [Packet,Num]
			% ),
...
prepare_and_exec (State, Name, Version, Stmt, Params) ->
    ...
    case do_query(State,StmtBin) of
		{updated,_} ->
			State1 =State #state{prepares = gb_trees:enter(Name,Version,State #state.prepares)},
...
normalize_version ([$4,$.,$0|_T], LogFun) ->
    mysql:log(LogFun,debug,"switching to MySQL 4.0.x protocol."),
    ?MYSQL_4_0;
...
normalize_version (_Other, LogFun) ->
    mysql:log(LogFun,error,"MySQL version not supported: MySQL Erlang module might not work correctly."),
